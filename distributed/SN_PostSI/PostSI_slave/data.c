...
void ProcessUpdateWirteList(uint64_t * recv_buffer, int conn, int sindex)
{
   int i;
   int tableid;
   uint64_t h;
...
void ProcessUpdateConflict(uint64_t * recv_buffer, int conn, int sindex)
{
   ...
   firstadd = recv_buffer[4];
   sid_max = recv_buffer[5];
   sid_min = recv_buffer[6];
   cid_min = recv_buffer[7];
   ...
   if (firstadd)
   {
	...
	if(!IsUpdateConflict(&HashTable[h], tid, sid_max, &sid_min, &cid_min))
	{
	    ...
	    if (SSend3(conn, sindex, status, sid_min, cid_min) == -1)
	    	printf("process update conflict send error\n");
	    return;
	}

	/*
	 * xxzhou: write-list-insert and merge-read-list.
	 */
	WriteListInsert(tableid, h, tid);

	*(ssend_buffer[sindex])=(uint64_t)status;
	*(ssend_buffer[sindex]+1)=(uint64_t)sid_min;
	*(ssend_buffer[sindex]+2)=(uint64_t)cid_min;

	for (i = 0; i < READLISTMAX; i++)
		*(ssend_buffer[sindex]+i+3) = ReadTransTable[tableid][i][h];

	if (send(conn, ssend_buffer[sindex], (READLISTMAX+3)*sizeof(uint64_t), 0) == -1)
		printf("process update write list insert send error\n");
}
...
void ProcessRestPair(uint64_t * recv_buffer, int conn, int sindex)
{
	int windex;
	int rindex;
	int status = 0;
