...
/*
 * following are implemented according to TPCC benchmark.
 */
	...
	for(w=1;w<=numWarehouses;w++)
		for(d=1;d<=DistPerWhse;d++)
			for(c=1;c<=CustPerDist;c++)
			{
				...
				k++;
				//add to avoid too many tuples operated in one transaction.
				if(k%configCommitCount==0)
				{
					result=PreCommit(&index);
					if(result == 1)
...
int paymentTransaction(int w_id, int c_w_id, int h_amount, int d_id, int c_d_id, int c_id)
{
	...
	else//good credit
	{
		...
		Data_Update(Customer_ID, cust_id, cust_value);
		if(result == 0)
		{
...
int orderStatusTransaction(int w_id, int d_id, int c_id)
{
	...
	int ol_o_id, ol_w_id, ol_d_id, ol_number, ol_i_id, ol_supply_w_id, ol_quantity, ol_amount;
	int result, index;
	int transaction_result;
	int flag;
...
int GetMaxOid(int table_id, int w_id, int d_id, int c_id)
{
	...
	for(i=min;i<max;i++)
	{
		tuple_value=LocateData_Read(table_id, i, &tuple_id);
